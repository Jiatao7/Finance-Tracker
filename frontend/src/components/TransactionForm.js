...
export default function TransactionForm () {
    ...
    async function handleSubmit(formData) {
        ...
        const response = await fetch(`https://${process.env.REACT_APP_API_URL}/api/transactions`, {method: "POST", headers: {"Content-Type": "application/json", "Authorization": `Bearer ${token}`}, body: JSON.stringify(newTransaction)})
        ...
        if(!response.ok) {
            ...
            const response = await fetch(`https://${process.env.REACT_APP_API_URL}/api/users/${user._id}`, {method: "PATCH", headers: {"Content-Type": "application/json"}, body: JSON.stringify({balance: newBalance})})
